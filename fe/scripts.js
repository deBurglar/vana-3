...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById('product-popup').addEventListener('click', function(e) {
     ...
     // Show the Bihar districts dropdown when the Bihar button is clicked
     document.getElementById('bihar-btn').addEventListener('click', function() {
        const dropdown = document.getElementById('bihar-districts-dropdown');
        dropdown.style.display = (dropdown.style.display === 'none') ? 'block' : 'none';
    });

    // Event listener for selecting a district
    document.getElementById('districts').addEventListener('change', function() {
        ...
        alert('You selected: ' + selectedDistrict);
    });


    const productTypeDropdown = document.getElementById("product-type");
    const specificProductDropdown = document.getElementById("specific-product");
    const productNameInput = document.getElementById("product-name");
    
    // Object containing specific products for each product type
    const productOptions = {
        fruits: ['Banana', 'Apple', 'Mango', 'Orange'],
        vegetables: ['Tomato', 'Potato', 'Carrot', 'Spinach'],
        pulses: ['Lentils', 'Chickpeas', 'Peas', 'Black Gram'],
        'oil-crops': ['Soybean', 'Groundnut', 'Sunflower', 'Sesame'],
        cereals: ['Wheat', 'Rice', 'Barley', 'Maize'],
        stimulants: ['Tea', 'Coffee', 'Cocoa', 'Tobacco'],
        spices: ['Pepper', 'Turmeric', 'Ginger', 'Cardamom'],
        fibres: ['Cotton', 'Jute', 'Silk'],
        'fodder-crops': ['Alfalfa', 'Clover', 'Sorghum', 'Oats'],
   ...
   // Event listener for the main product type dropdown
   productTypeDropdown.addEventListener("change", function() {
    const selectedType = productTypeDropdown.value;

    // Enable and populate the second dropdown based on the selected product type
    if (productOptions[selectedType]) {
        specificProductDropdown.disabled = false; // Enable dropdown
        specificProductDropdown.innerHTML = ''; // Clear previous options

        // Add new options for the selected product type
        productOptions[selectedType].forEach(function(product) {
            const option = document.createElement('option');
            option.value = product.toLowerCase().replace(/\s+/g, '-'); // Create a value for the option
            option.textContent = product; // Set the display text
            specificProductDropdown.appendChild(option); // Append the option to the dropdown
        });
    } else {
        // If no specific products are available, disable the second dropdown
        specificProductDropdown.disabled = true;
        specificProductDropdown.innerHTML = ''; // Clear the dropdown options
        specificProductDropdown.innerHTML = '<option value="" disabled selected>No products available</option>';
    } 
    });
});
...
document.querySelector('button[type="submit"]').addEventListener('click', function(event) {
    ...
    //new
    if (!productType || !specificProduct || !productWeight) {
